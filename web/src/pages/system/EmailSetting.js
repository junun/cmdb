    ...
    return (
      <React.Fragment>
        ...
        <Form style={{maxWidth: 340}}>
          ...
          <div>
            {hasPermission('system.email.test') &&
              <Button type="danger" loading={this.state.mail_test_loading} style={{ marginRight: '10px' }}
                     onClick={this.handleEmailTest}>测试邮件服务</Button>}
            {hasPermission('system.setting.edit') &&
              <Button type="primary" loading={loading} style={{ marginTop: 20}} onClick={this.handleSubmit}>保存设置</Button>}
          </div>
        </Form>
      </React.Fragment>
    )
