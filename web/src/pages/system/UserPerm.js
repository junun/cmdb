...
class UserPerm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  componentDidMount() {
    this.props.dispatch({
      type: 'system/getUserPerm',
      payload: this.props.uid
    }).then(()=> {
      this.setState({
        temp: this.props.userPermList,
        oldUserPermCount: this.props.userPermList.length
      });
    });
  ...
  handleAllCheck = (e, mod, page) => {
    ...
    let userPermListTemp = this.state.temp;
    const key = "index" +`${mod}` + `${page}`;
    if (tmp.hasOwnProperty(key)) {
      tmp[key].map((item) => {
        let index = userPermListTemp.indexOf(item);
        if (checked) {
          if (index === -1 ) {
            // 添加
            userPermListTemp.push(item)
          }
        } else {
          if (index > -1 ) {
            // 减少
            userPermListTemp.splice(index, 1);
          }
        }
      });
    } else {
  ...
  handlePermOk = () => {
    ...
    if (keys.length > 0 || keys.length !== this.state.oldUserPermCount) {
      const values = {};
      values.id = this.props.uid;
      values.code = keys;
      this.props.dispatch({
        type: 'system/userPermAdd',
        payload: values,
      });
    } else {
